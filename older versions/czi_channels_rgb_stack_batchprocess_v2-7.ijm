...
for (i = 0; i < list.length; i++) {
	...
	if (endsWith(list[i], ".czi")) { // process only .czi files
	    ...
	    print("file opened");
	    // Get the number of channels in the current image
		//nChannels = getImageProperty("channels");
		//nChannels = Stack.getDimensions()[2];
		//a, b, nChannels, c, d = getDimensions();
		getDimensions(dummy, dummy, nChannels, dummy, dummy);
		
		if (nChannels < ch) {
	        print("File " + file_name + " presents " + nChannels + " channels and was skipped");
	    ...
	    // Iterate over channels, but not beyond the maximum applicable number of channels
		for (ii = 1; ii <= ch; ii++) {
	    	...
	    	if (ii != nc82) {					// nc82 input by the user
	    		...
	    		rename(file_name); //renames recently created merge
